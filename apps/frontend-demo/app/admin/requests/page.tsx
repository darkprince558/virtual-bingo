  ...
  return (
    <DashboardShell role="admin" userName="Admin Team">
      <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
        ...
        {/* Pending section */}
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="rounded-3xl p-5 sm:p-6 mb-6" style={{ background: '#FFFFFF', border: '1.5px solid #F0EDE8', boxShadow: '0 2px 16px rgba(0,0,0,0.04)' }}>
          ...
          {pending.length === 0 ? (
            ...
            <div className="space-y-3">
              {pending.map((req, i) => (
                <motion.div key={req.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.05 }} className="rounded-2xl p-4 sm:p-5" style={{ background: '#FAFAF9', border: '1.5px solid #F0EDE8' }}>
                  <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 text-sm font-black" style={{ background: 'linear-gradient(135deg, #FFA070, #FF5A1F)', color: '#FFFFFF' }}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs font-semibold mt-2 px-3 py-2 rounded-xl" style={{ background: '#F4F2EF', color: '#57534E' }}>
                          {`"${req.reason}"`}
                        </p>
                    ...
                    <div className="flex gap-2 shrink-0">
                      <motion.button whileTap={{ scale: 0.95 }} onClick={() => approve(req.id)} className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-xs font-extrabold transition-all" style={{ background: 'linear-gradient(135deg, #3DC484, #22AA6A)', color: '#fff', boxShadow: '0 3px 10px rgba(34,170,106,0.25)' }}>
                        <Check className="w-4 h-4" /> Approve
                      </motion.button>
                      <motion.button whileTap={{ scale: 0.95 }} onClick={() => reject(req.id)} className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-xs font-extrabold transition-all" style={{ background: '#FFE4E6', color: '#BE123C', border: '1.5px solid #FECDD3' }}>
        ...
        {/* Resolved section */}
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="rounded-3xl p-5 sm:p-6" style={{ background: '#FFFFFF', border: '1.5px solid #F0EDE8', boxShadow: '0 2px 16px rgba(0,0,0,0.04)' }}>
          <h2 className="text-sm font-extrabold uppercase tracking-widest mb-4" style={{ color: '#A8A29E' }}>History</h2>
          <div className="space-y-2.5">
            {resolved.map((req, i) => (
              <div key={req.id} className="flex items-center gap-3 px-4 py-3 rounded-2xl" style={{ background: '#FAFAF9', border: '1.5px solid #F0EDE8' }}>
                <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: req.status === 'Approved' ? '#D5F5E6' : '#FFE4E6' }}>

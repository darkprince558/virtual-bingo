      ...
      <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
        ...
        <div className="space-y-4">
          {profiles.map((profile, i) => {
            ...
            return (
              <motion.div key={profile.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.06 }} className="rounded-xl p-5 sm:p-6" style={{ background: '#FFFFFF', border: '1.5px solid #F0EDE8', boxShadow: '0 2px 16px rgba(0,0,0,0.04)' }}>
                <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                  ...
                  <div className="flex gap-2 shrink-0">
                    ...
                    {profile.status === 'Pending Consent' && (
                      <span className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-md text-xs font-bold w-[116px]" style={{ background: '#FEF3C7', color: '#B45309', border: '1.5px solid #FDE68A' }}>
                        <Clock className="w-3.5 h-3.5 shrink-0" /> Awaiting
                      </span>
                    )}
                    {profile.status === 'Revoked' && (
                      <span className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-md text-xs font-bold w-[116px]" style={{ background: '#FFE4E6', color: '#BE123C', border: '1.5px solid #FDA4AF' }}>

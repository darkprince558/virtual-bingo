...
function PlayContent() {
  ...
  useEffect(() => {
    if (snapshot?.gameRun?.status?.toLowerCase() === 'finished' && !audioPlayedRef.current) {
      audioPlayedRef.current = true
      try {
        const isWinner = snapshot.winners?.some(w => w.playerId === snapshot.player?.id)
        const text = isWinner ? "Bingo! Congratulations you won!" : "Bingo! We have a winner!"
    ...
    if (snapshot?.gameRun?.status?.toLowerCase() !== 'finished') {
      audioPlayedRef.current = false
    }
  }, [snapshot?.gameRun?.status, snapshot?.winners, snapshot?.player?.id])
  ...
  const cardToCells = useCallback((card: CardResponse): BingoCellData[] => (
    card.cells.map(c => ({
      id: c.id,
      word: c.word,
      isMarked: !!c.markedAt || c.isFreeSpace,
  ...
  const navHeight = viewport && viewport.width < 640 ? 56 : 68
  const playAreaHeight = viewport ? `${Math.max(viewport.height - navHeight, 360)}px` : 'calc(100vh - 68px)'
  const aiHostMessage = confirmedWinner
    ? `${snapshot.player.displayName} has a confirmed ${mapBingoPattern(confirmedWinner.pattern)} bingo.`
    : currentWord
      ? `Now tracking ${snapshot.player.displayName}'s card after "${currentWord.word}". ${claimReadiness?.ready ? 'A valid claim is ready.' : 'Keep watching the called words.'}`
      : `${snapshot.player.displayName}'s card is loaded. Waiting for the first word call.`

  return (
    <AppShell>
      <TopNav
        gameId={snapshot.gameRun.id}
      ...
      <main
        className="min-h-0 flex flex-col lg:flex-row overflow-hidden relative"
      ...
      >
        ...
        {/* ── Main Game Area ── */}
        <div
          className="flex-1 min-h-0 p-4 sm:p-6 lg:p-6 xl:p-8 flex flex-col gap-4 lg:gap-5 overflow-y-auto relative z-10"
        ...
        >
          ...
          <div className="shrink-0">
            <CurrentCallDisplay
              ...
              callNumber={snapshot.calledWords.length}
              audioUrl={snapshot.currentCallerAsset?.audioUrl}
            />
          </div>

          {missedWords.length > 0 && (
            <div className="shrink-0 rounded-lg p-4" style={{ background: '#FFFBEB', border: '1.5px solid #FDE68A', color: '#92400E' }}>
        ...
        >
          ...
          <div className="p-5 border-b border-[#F4F2EF]">
            <Leaderboard entries={snapshot.winners.map(w => ({ placement: toPodiumPlacement(w.placement), player: { id: w.playerId, name: w.playerId === snapshot.player.id ? snapshot.player.displayName : 'Player', state: 'Confirmed Winner', connectionState: 'Connected' }, wordsMatched: w.pattern === 'full_house' ? 24 : w.pattern === 'four_corners' ? 4 : 5 }))} />
          </div>
          ...
          {/* AI Host panel */}
          <div className="shrink-0">
            <AIHostPanel message={aiHostMessage} />
          </div>
        </aside>
      </main>
